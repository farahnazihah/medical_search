...
class InvertedIndex:
    ...
    def __init__(self, index_name, postings_encoding, directory=''):
        ...
        self.index_file_path = staticfiles_storage.url(
            f'{directory}/{index_name}.index')[1:]
        self.metadata_file_path = staticfiles_storage.url(
            f'{directory}/{index_name}.dict')[1:]
    ...
    def __enter__(self):
        ...
        with open(self.metadata_file_path, 'rb') as f:
            self.postings_dict, self.terms, self.doc_length = pickle.load(
                File(f))
...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        postings_list = self.postings_encoding.decode(
            self.index_file.read(length_post))
        tf_list = self.postings_encoding.decode_tf(
            self.index_file.read(length_tf))
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        start_pos = self.postings_dict[last_term][0] + self.postings_dict[last_term][2] + \
            self.postings_dict[last_term][3] if last_term is not None else 0
        postings = self.postings_encoding.encode(postings_list)
        tf = self.postings_encoding.encode_tf(tf_list)
        self.postings_dict[term] = (start_pos, len(
            postings_list), len(postings), len(tf))
