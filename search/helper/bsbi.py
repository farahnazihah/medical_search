from medical_search.settings import BASE_DIR
...
class BSBIIndex:
    ...
    #                         doc_id = self.doc_id_map[os.path.join(
    #                             block_dir_relative, doc_name)]
    ...
    #                 for t in query_term if not t.lower() in stop_words]
    #     heap = []

    #     with InvertedIndexReader(self.index_name, directory=self.output_dir, postings_encoding=self.postings_encoding) as mapper:
    ...
    #             postings_list, tf_list = mapper.get_postings_list(
    #                 self.term_id_map[term])
    #             for i in range(df):
    #                 wtd = 0
    #                 if tf_list[i] > 0:
    ...
    #                     (self.doc_id_map[postings_list[i]], score))
    ...
    #     heap_res = sorted(heap[0], key=lambda t: t[1]
    #                       )      # sort based on score
    #     result = heap_res[-1:-k-1:-1]                       # retrieve k-top
    #     # reverse tuple element to (score, doc)
    ...
    def load(self):
        ...
        term_str_to_id_path = staticfiles_storage.url(
            f'{self.output_dir}/terms_str_to_id.dict')[1:]
        with open(term_str_to_id_path, 'rb') as f:
            self.term_id_map.str_to_id = pickle.load(File(f))
        term_id_to_str_path = staticfiles_storage.url(
            f'{self.output_dir}/terms_id_to_str.dict')[1:]
        with open(term_id_to_str_path, 'rb') as f:
            self.term_id_map.id_to_str = pickle.load(File(f))

        doc_str_to_id_path = staticfiles_storage.url(
            f'{self.output_dir}/docs_str_to_id.dict')[1:]
        with open(doc_str_to_id_path, 'rb') as f:
            self.doc_id_map.str_to_id = pickle.load(File(f))
        doc_id_to_str_path = staticfiles_storage.url(
            f'{self.output_dir}/docs_id_to_str.dict')[1:]
        ...
        with InvertedIndexReader(self.index_name, directory=self.output_dir, postings_encoding=self.postings_encoding) as mapper:
            ...
            for term in filtered:
                ...
                for i in range(df):
                    ...
                    wtd = ((k1 + 1) * tf_list[i]) / (k1 *
                                                     ((1 - b) + b * dl/avdl) + tf_list[i])
                    score = wtq * wtd
                    scores_per_doc.append(
                        (self.doc_id_map[postings_list[i]], score))
                heapq.heappush(heap, scores_per_doc)
